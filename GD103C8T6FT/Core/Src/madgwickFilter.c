...
struct quaternion q_est_rp = { 1, 0, 0, 0};       // initialize with as unit vector with real component  = 1

struct quaternion q_est_y = { 1, 0, 0, 0};       // initialize with as unit vector with real component  = 1
...
void imu_filter_y(float ax, float ay, float az, float gx, float gy, float gz){
    ...
    struct quaternion q_est_prev = q_est_y;
    ...
    */
    ...
    quat_scalar(&q_est_dot, DELTA_T_AUX);
    quat_add(&q_est_y, q_est_prev, q_est_dot);     // Integrate orientation rate to find position
    quat_Normalization(&q_est_y);                 // normalize the orientation of the estimate
...
void imu_filter_rp(float ax, float ay, float az, float gx, float gy, float gz){
    ...
    //Variables and constants
    struct quaternion q_est_prev = q_est_rp;
    struct quaternion q_est_dot = {0};            // used as a place holder in equations 42 and 43
    //const struct quaternion q_g_ref = {0, 0, 0, 1};// equation (23), reference to field of gravity for gradient descent optimization (not needed because I used eq 25 instead of eq 21
    struct quaternion q_a = {0, ax, ay, az};    // equation (24) raw acceleration values, needs to be normalized
    ...
    */
    ...
    quat_add(&q_est_rp, q_est_prev, q_est_dot);     // Integrate orientation rate to find position
    quat_Normalization(&q_est_rp);                 // normalize the orientation of the estimate
                                                //(shown in diagram, plus always use unit quaternions for orientation)
