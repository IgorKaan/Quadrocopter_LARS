...
#define MS5611_EN HAL_GPIO_WritePin(NSS_MS_GPIO_Port, NSS_MS_Pin, GPIO_PIN_RESET);
#define MS5611_DIS HAL_GPIO_WritePin(NSS_MS_GPIO_Port, NSS_MS_Pin, GPIO_PIN_SET);
...
static uint32_t convDelay = CONVERSION_OSR_4096;

static int32_t temperature;
static int32_t pressure;
static float altitude;
volatile static float altitudeOffset;
static float referencePressure;
volatile float relativeAltitude;
volatile float altitudeCorrected;
volatile float relativeAltitudeFiltered;
 ...
 */
uint16_t ms5611_read16bits(uint8_t reg);
...
//	c = 1.0/5.255;
//	altitude = (1 - pow(r,c))*44330.77;
//	altitude_corrected = altitude - altitude_offset;
//}
